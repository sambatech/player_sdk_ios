...
class MediaListViewController : UITableViewController {
	...
	private func requestMediaSet(pids:[String]) {
		...
		func request() {
			let pid = pids[i]
			let url = "\(Commons.settings["svapi_endpoint"]!)medias?access_token=\(Commons.settings["svapi_token"]!)&pid=\(pid)&published=true"
	...
	private func requestAds(hash: String) {
		let url = "\(Commons.settings["myjson_endpoint"]!)\(hash)"
		Alamofire.request(.GET, url).responseJSON { response in
			guard let json = response.result.value as? [AnyObject] else {
				print("Error: Invalid JSON format!")
				return
